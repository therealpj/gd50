...
function Paddle:init(skin)
    ...
    self.quad = gFrames['paddles'][self.size + 4 * (self.skin - 1)]

    --gTextures['main'], gFrames['paddles'][self.size + 4 * (self.skin - 1)]
end
...
function Paddle:render()
   ...
   love.graphics.draw(gTextures['main'], self.quad,
	self.x, self.y)
end
...
function Paddle:updatePaddle()
   self.quad = gFrames['paddles'][self.size + 4 * (self.skin - 1)]
   lx, ly, lw, lh = self.quad:getViewport()
   self.width = lw
   self.height = lh
end
