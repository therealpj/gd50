...
containsLockedBrick = false
spawnLockedBrick = false
...
function LevelMaker.createMap(level)
   -- one in three chance that this level contains a locked brick
 --  if math.floor(love.timer.getTime()) % 1 == 0 then
   --   print("here1")
     -- spawnLockedBrick = true
   --end
	...
	for x = 1, numCols do
		...
		* 32                    -- multiply by 32, the brick width
	    ...
	    -- change this brick to a locked brick with a chance, only if this level doesnt contain one and hasnt spawned one
	    if math.floor(love.timer.getTime() * 10000) % 13 == 0 and containsLockedBrick == false then
	       print("here")
	       b.isLocked = true
	       containsLockedBrick = true
	       b.tier = 0
	       b.color = 1
	    end
